...
function ShalConnectsPortfolio() {
  ...
  // Intersection Observer for service cards (staggered animations)
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
  ...
  // Handle hash navigation and service pre-fill
  useEffect(() => {
    // Handle hash navigation to contact section
    if (location.hash === '#contact' && !hasScrolledToContactRef.current) {
      ...
      // Reset the ref when hash changes away from #contact
      hasScrolledToContactRef.current = false;
    }

    // Handle service pre-fill from navigation state
    if (location.state?.prefillService) {
      setFormData((prev) => ({
        ...prev,
        service: location.state.prefillService
      }));
      // Clear the state to prevent re-filling on re-renders, but preserve the hash
      const newPath = location.pathname + location.search + location.hash;
      navigate(newPath, { replace: true, state: null });
    }
  }, [location.hash, location.state, navigate]);

  // Outside click detection for More menu dropdown
  useEffect(() => {
    ...
    if (isMoreMenuOpen || isServiceDropdownOpen) {
  ...
  }, [isMoreMenuOpen, isServiceDropdownOpen]);

  // Update dropdown position when menu opens
  useEffect(() => {
    if (isMoreMenuOpen && moreMenuRef.current) {
      const updatePosition = () => {
        // Try to get the button element inside the wrapper for more accurate positioning
        const buttonElement = moreMenuRef.current?.querySelector('button');
        const rect = buttonElement?.getBoundingClientRect() || moreMenuRef.current?.getBoundingClientRect();
        if (rect) {
          const dropdownWidth = 180; // Approximate dropdown width
          const dropdownHeight = 150; // Approximate dropdown height (3 items + padding)
          ...
          // If dropdown would go off-screen to the right, align it to button's right edge instead
          ...
          // For desktop bottom navigation, position dropdown above the button
          // For mobile, position below the button
          let top;
          if (isDesktop) {
            // Desktop: position directly above the button with small gap
            top = rect.top - dropdownHeight - gap;
            // If it would go off-screen at the top, position below instead
            if (top < 8) {
              top = rect.bottom + gap;
            }
          } else {
            ...
            top = rect.bottom + gap;
            // If it would go off-screen at the bottom, position above instead
  ...
  useEffect(() => {
    ...
    const animateStats = () => {
      ...
      const duration = 500; // Very fast animation - 500ms
      const steps = 60;
      ...
      (Object.keys(statsTarget) as Array<keyof Stats>).forEach((key) => {
        ...
        const timer = setInterval(() => {
          ...
          if (current >= statsTarget[key]) {
            setStats((prev) => ({ ...prev, [key]: statsTarget[key] }));
            clearInterval(timer);
          } else {
    ...
    // Use a more reliable check with a slight delay to ensure DOM is ready
    const checkAndAnimate = () => {
      const heroSection = document.getElementById('home');
      if (heroSection && !hasAnimated) {
    ...
    // Observe both hero section and stats section
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
    ...
    // Observe hero section (where quick stats are displayed)
    const heroSection = document.getElementById('home');
    if (heroSection) {
      observer.observe(heroSection);
    }

    // Also observe stats section for when it comes into view
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const interactionRadius = 20; // Radius for size/color effects

      particlesRef.current = particlesRef.current.map((particle) => {
        let { x, y, vx, vy, baseSize, baseOpacity } = particle;
        ...
        // Size dynamics based on mouse proximity
        let sizeMultiplier = 1;
        if (distance < interactionRadius) {
          const proximityFactor = 1 - (distance / interactionRadius);
          sizeMultiplier = 1 + proximityFactor * 0.8; // Grow up to 1.8x
        }
        ...
        if (distance < interactionRadius) {
          const proximityFactor = 1 - (distance / interactionRadius);
          colorIntensity = 1 + proximityFactor * 0.5; // Increase intensity up to 1.5x
        }

        // Opacity enhancement near mouse
        let currentOpacity = baseOpacity;
        if (distance < interactionRadius) {
          const proximityFactor = 1 - (distance / interactionRadius);
          currentOpacity = Math.min(1, baseOpacity + proximityFactor * 0.3);
        }
      ...
      const connections = [];
      const maxConnectionDistance = 8; // Maximum distance for connections
        ...
        for (let j = i + 1; j < particlesRef.current.length; j++) {
          ...
          if (distance < maxConnectionDistance) {
            connections.push({
              ...
              to: { x: p2.x, y: p2.y },
              distance,
              opacity: (1 - distance / maxConnectionDistance) * 0.3,
            });
          }
        }
      }
      
      setParticleConnections(connections);
  ...
  const allServices = serviceCategories.flatMap(category => 
    category.services.map(service => ({
      value: `${category.name} - ${service.title}`,
      label: service.title,
      category: category.name
    }))
  );
  allServices.push({ value: 'General Inquiry', label: 'General Inquiry', category: 'Other' });
  ...
  const projects = [
    { 
      ...
      color: 'linear-gradient(to bottom right, #176641, #da651e)',
      description: 'Complete brand transformation for a tech startup, including logo design, brand guidelines, and digital presence.',
      results: '300% increase in brand recognition, 150% growth in user acquisition',
    ...
    { 
      ...
      color: 'linear-gradient(to bottom right, #176641, #1e7a4f)',
      description: 'Multi-channel digital marketing campaign that increased engagement and drove sustainable growth.',
      results: '250% ROI, 500K+ impressions, 40% conversion rate increase',
    ...
    { 
      ...
      results: '180% increase in sales, 60% reduction in bounce rate',
      services: ['Web Design', 'UX/UI Design', 'E-commerce Development']
    },
    { 
      id: 4,
      title: 'FitLife Pro', 
      ...
      description: 'Mobile app design and development for fitness enthusiasts with social features.',
      results: '100K+ downloads, 4.8 star rating, 70% user retention',
      services: ['App Design', 'UI/UX', 'Brand Identity']
    }
  ];
  ...
  const testimonials = [
    {
      ...
      content: 'ShalConnects transformed our brand identity completely. Their strategic approach and creative execution exceeded all expectations.',
      rating: 5
    },
    {
      name: 'Michael Chen',
      role: 'Marketing Director, GreenEarth',
      content: 'The digital campaign they created for us was outstanding. We saw immediate results and continue to benefit from their expertise.',
      rating: 5
    },
    {
      name: 'Emily Rodriguez',
      role: 'Founder, UrbanStyle',
      content: 'Working with ShalConnects was a game-changer. They understood our vision and brought it to life beautifully.',
      rating: 5
    }
  ];
  ...
  const processSteps = [
    ...
    { 
      icon: Workflow, 
      title: 'Strategy', 
      desc: 'Crafting a strategy tailored to your objectives.',
      timeline: 'Week 2',
      subSteps: [
        'Strategic planning',
      ...
      keyQuestions: [
        'What platforms serve your audience?',
    ...
    { 
      ...
      desc: 'Bringing the strategy to life with design and development.',
      timeline: 'Weeks 3-4',
      subSteps: [
        'Wireframing',
        'Visual design',
      ...
      keyQuestions: [
        'How should design reflect your brand?',
        'What features are essential?',
        'What is the optimal user flow?',
    ...
    { 
      ...
      desc: 'Launching and continuously optimizing for results.',
      timeline: 'Week 4+',
      subSteps: [
        'Pre-launch testing',
        'Deployment',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/mdkbevlz', {
        method: 'POST',
        headers: {
      ...
      alert('Sorry, there was an error sending your message. Please try again or contact us directly via WhatsApp or email.');
  ...
  // Main portfolio content component - memoized to prevent infinite re-renders
  ...
  ), [scrollProgress, scrollY, isMenuOpen, activeTab, activeServiceCard, visibleServiceCards, serviceCategories, formData, formErrors, isSubmitting, navigate]);
